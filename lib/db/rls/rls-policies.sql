...
-- 辅助函数：从会话变量获取当前认证用户的UUID (Supabase Auth ID)
...
-- 辅助函数：获取当前Supabase Auth用户在public.users表中的内部ID
...
AS $$
DECLARE
    auth_user_uuid uuid; -- Supabase Auth 的用户ID
    app_internal_uuid uuid; -- applications.users 表的 ID
BEGIN
    auth_user_uuid := public.get_user_id();

    IF auth_user_uuid IS NULL THEN
        RETURN NULL;
    END IF;

    -- 在public.users表中根据supabaseId查找对应的内部ID
    SELECT id INTO app_internal_uuid
    FROM public.users
    WHERE supabaseId = auth_user_uuid;

    RETURN app_internal_uuid;
EXCEPTION WHEN OTHERS THEN
    RETURN NULL; -- 查找失败或会话变量未设置
END;
$$;
...
-- SELECT策略：用户可以读取自己的个人资料 (通过 supabaseId 关联)，或者系统管理员可以读取所有用户
DROP POLICY IF EXISTS "Users can read their own profile" ON public.users;
CREATE POLICY "Users can read their own profile"
ON public.users FOR SELECT
-- 使用 supabaseId 字段来匹配 get_user_id()，因为 id 是内部 UUID
USING ( public.get_user_id() = supabaseId OR public.get_user_jwt_role() = 'admin' );

-- INSERT策略：用户可以创建自己的个人资料（通常在注册过程中处理）
DROP POLICY IF EXISTS "Users can create their own profile" ON public.users;
CREATE POLICY "Users can create their own profile"
ON public.users FOR INSERT
-- 使用 supabaseId 字段来匹配 get_user_id()
WITH CHECK (public.get_user_id() = supabaseId);
...
-- 使用 supabaseId 字段来匹配 get_user_id()
USING (public.get_user_id() = supabaseId)
WITH CHECK (public.get_user_id() = supabaseId);
...
USING ( id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() ) );

-- INSERT策略：任何认证用户都可以创建团队
DROP POLICY IF EXISTS "Authenticated users can create teams" ON public.teams;
CREATE POLICY "Authenticated users can create teams"
ON public.teams FOR INSERT
-- 只要能获取到应用程序内部用户ID，就表示已认证
WITH CHECK (public.get_app_user_id() IS NOT NULL);

-- UPDATE策略：只有团队所有者可以更新他们的团队
DROP POLICY IF EXISTS "Team owners can update their teams" ON public.teams;
CREATE POLICY "Team owners can update their teams"
ON public.teams FOR UPDATE
USING ( id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) )
WITH CHECK ( id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
USING ( user_id = public.get_app_user_id() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );

-- INSERT策略：只有团队所有者可以添加新成员
DROP POLICY IF EXISTS "Team owners can add members" ON public.team_members;
CREATE POLICY "Team owners can add members"
ON public.team_members FOR INSERT
WITH CHECK ( team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
USING ( user_id = public.get_app_user_id() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) )
WITH CHECK ( user_id = public.get_app_user_id() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
USING ( email = public.get_user_email() OR invited_by = public.get_app_user_id() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
WITH CHECK ( invited_by = public.get_app_user_id() AND team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
-- team_members.user_id 引用的是 users.id，所以需要 get_app_user_id()
USING ( email = public.get_user_email() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) )
WITH CHECK ( email = public.get_user_email() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
USING ( invited_by = public.get_app_user_id() OR team_id IN ( SELECT team_id FROM public.team_members WHERE user_id = public.get_app_user_id() AND role = 'owner' ) );
...
WITH CHECK ( ( public.get_app_user_id() = author_id AND is_anonymous = false ) OR ( author_id = '00000000-0000-0000-0000-000000000001' AND is_anonymous = true ) );

-- UPDATE策略：只有作者可以更新他们的非匿名帖子
DROP POLICY IF EXISTS "Authors can update their posts" ON public.posts;
CREATE POLICY "Authors can update their posts"
ON public.posts FOR UPDATE
USING ( public.get_app_user_id() = author_id AND is_anonymous = false )
WITH CHECK ( public.get_app_user_id() = author_id AND is_anonymous = false );
